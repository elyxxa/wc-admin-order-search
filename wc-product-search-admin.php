...
// IMPORTANT: This plugin uses a compatibility layer to handle namespace changes
// The original plugin code used namespace WC_Order_Search_Admin
// This version uses namespace WC_Product_Search_Admin
// When debugging issues, please note this change
...
add_action(
    ...
    function () {
        ...
        // Our AlgoliaSearch compatibility classes
        require_once WC_PSA_PATH . 'includes/AlgoliaSearch/AlgoliaException.php';
        require_once WC_PSA_PATH . 'includes/AlgoliaSearch/ClientContext.php';
        require_once WC_PSA_PATH . 'includes/AlgoliaSearch/Client.php';
        require_once WC_PSA_PATH . 'includes/AlgoliaSearch/Index.php';
        require_once WC_PSA_PATH . 'includes/AlgoliaSearch/Version.php';
        ...
        $plugin = \WC_Product_Search_Admin\Plugin::initialize(new \WC_Product_Search_Admin\Options());
        ...
        if (is_admin()) {
            ...
            new \WC_Product_Search_Admin\Admin\Options_Page($plugin->get_options());
            new \WC_Product_Search_Admin\Admin\Products_List_Page($plugin->get_options());
            if ($plugin->get_options()->has_algolia_account_settings()) {
                new \WC_Product_Search_Admin\Admin\Ajax_Reindex($plugin->get_products_index(), $plugin->get_options());
            }
            new \WC_Product_Search_Admin\Admin\Ajax_Indexing_Options_Form($plugin->get_options());
            new \WC_Product_Search_Admin\Admin\Ajax_Algolia_Account_Settings_Form($plugin->get_options());
        }
        ...
        if (defined('WP_CLI') && WP_CLI && $plugin->get_options()->has_algolia_account_settings()) {
            ...
            $commands = new \WC_Product_Search_Admin\Commands($plugin->get_products_index(), $plugin->get_options());
