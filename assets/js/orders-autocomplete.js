...
// Check which search input is present on the page
var searchInputSelector = '#post-search-input';
if (jQuery(searchInputSelector).length === 0) {
    // Try WooCommerce's default search input
    searchInputSelector = '#orders-search-input-search-input';

    // If still not found, try other common selectors
    if (jQuery(searchInputSelector).length === 0) {
        searchInputSelector = '.search-box input[name="s"]';

        // If neither exists, create our own search input
        if (jQuery(searchInputSelector).length === 0) {

            jQuery('.search-box').prepend(
                '<div class="alignleft actions algolia-search-wrapper">' +
                '<label for="algolia-search-input" class="screen-reader-text">Search Orders with Algolia</label>' +
                '<input type="search" id="algolia-search-input" placeholder="Search orders with Algolia" class="search-input" style="margin-right: 6px;">' +
                '</div>'
            );
...
autocomplete(searchInputSelector, {hint: false, debug: aosOptions.debug}, [
    {
...
// Add some custom CSS for our search box
jQuery('head').append('<style>' +
    '.algolia-search-wrapper { position: relative; margin-bottom: 10px; }' +
    '.aa-dropdown-menu { background-color: #fff; border: 1px solid #d6d6d6; box-shadow: 0 1px 5px rgba(0,0,0,0.1); margin-top: 3px; width: 100%; }' +
    '.aa-dropdown-menu a { color: #444; display: block; padding: 8px; text-decoration: none; }' +
    '.aa-dropdown-menu a:hover { background-color: #f5f5f5; }' +
    '.algolia-autocomplete { width: 300px; }' +
    '</style>');
