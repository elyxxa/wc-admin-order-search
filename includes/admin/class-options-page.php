...
class Options_Page
{
    ...
    public function enqueue_scripts()
    {
        ...
        $suffix = '';
        wp_enqueue_script('wc_psa_products_search', plugin_dir_url(WC_PSA_FILE) . 'assets/js/reindex-products-button' . $suffix . '.js', array( 'jquery' ), WC_PSA_VERSION, true);
        wp_enqueue_script('wc_psa_ajax_forms', plugin_dir_url(WC_PSA_FILE) . 'assets/js/ajax-forms' . $suffix . '.js', array( 'jquery' ), WC_PSA_VERSION, true);
    }

    public function register_page_in_menu()
    {
        add_options_page(
            __('WooCommerce Product Search Admin', 'wc-product-search-admin'),
            __('WooCommerce Product Search Admin', 'wc-product-search-admin'),
            'manage_options',
            'wc_psa_options',
